(() => {
  ...
  const decisionRunKey='flt-v38-decision-run-this-view';
  ...
  const decisionAlreadyRun=()=>sessionStorage.getItem(decisionRunKey)==='true';
  ...
  document.addEventListener('input',e=>{if(e.target.matches('input,select,textarea')){if(decisionInputIds.has(e.target.id))sessionStorage.removeItem(decisionRunKey);schedule(false)}},true);
  document.addEventListener('change',e=>{if(e.target.matches('input,select,textarea')){if(decisionInputIds.has(e.target.id))sessionStorage.removeItem(decisionRunKey);schedule(true)}},true);
  document.addEventListener('submit',e=>{if(e.target.id==='v35-decision-form'){sessionStorage.setItem(decisionRunKey,'true');setTimeout(()=>schedule(false),80)}},true);
