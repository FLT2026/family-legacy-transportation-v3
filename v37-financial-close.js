(() => {
  ...
  function reconcile(load){
    const invoice=invoiceAmount(load),payments=paymentAmount(load),expenses=expenseAmount(load),receivable=invoice-payments,journal=[],records=paymentRecords(load);
    ...
    return {loadId:load?.id||'',podComplete:Boolean(load?.pod&&load?.deliveryProof?.signature),invoiceNumber:load?.invoice?.number||'',invoice,payments,receivable,expenses,profit:invoice-expenses,debits,credits,balanced:debits===credits,linesEqualInvoice,allocationsMatch,journal};
  }
