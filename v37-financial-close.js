(() => {
  ...
  const paymentAmount=load=>sum(Array.isArray(load?.payments)?load.payments:[],item=>item.amount);
  const expenseAmount=load=>sum(Array.isArray(load?.expenses)?load.expenses:[],item=>item.amount);

  function reconcile(load){
    const invoice=invoiceAmount(load),payments=paymentAmount(load),expenses=expenseAmount(load),receivable=invoice-payments,journal=[];
    if(load?.invoice?.number&&invoice){
      journal.push({source:'invoice',account:'Accounts Receivable',debit:invoice,credit:0});
      journal.push({source:'invoice',account:'Transportation Revenue',debit:0,credit:invoice});
    }
    (load?.payments||[]).forEach((payment,index)=>{
      const amount=cents(payment.amount);if(!amount)return;
      journal.push({source:'payment-'+index,account:'Cash / Undeposited Funds',debit:amount,credit:0});
      journal.push({source:'payment-'+index,account:'Accounts Receivable',debit:0,credit:amount});
    });
    ...
    return {loadId:load?.id||'',podComplete:Boolean(load?.pod&&load?.deliveryProof?.signature),invoiceNumber:load?.invoice?.number||'',invoice,payments,receivable,expenses,profit:invoice-expenses,debits,credits,balanced:debits===credits,journal};
  }
  ...
  function renderFinancialClose(){
    ...
    const result=reconcile(current()),isolated=isolationCheck(store?.loads||[]);
    ...
    if(panel)panel.innerHTML='<div class="section-head"><div><div class="eyebrow">V3.7 financial close</div><h2>Load-ID reconciliation</h2><p class="subtle" style="margin-top:5px">Read-only foundation for '+escapeHtml(result.loadId||'no selected load')+'.</p></div>'+tag(result.balanced&&isolated?'RECONCILED':'REVIEW',result.balanced&&isolated)+'</div><div class="grid stats"><div class="stat"><span class="label">Invoice total</span><div class="value">'+display(result.invoice)+'</div></div><div class="stat"><span class="label">Payments</span><div class="value">'+display(result.payments)+'</div></div><div class="stat"><span class="label">Receivable</span><div class="value">'+display(result.receivable)+'</div></div><div class="stat"><span class="label">Earned profit</span><div class="value">'+display(result.profit)+'</div></div></div><div class="metric-row"><span>POD linked to this Load ID</span>'+tag(result.podComplete?'PASS':'PENDING',result.podComplete)+'</div><div class="metric-row"><span>Journal debits '+display(result.debits)+' = credits '+display(result.credits)+'</span>'+tag(result.balanced?'PASS':'FAIL',result.balanced)+'</div><div class="metric-row"><span>Finance isolation across saved loads</span>'+tag(isolated?'PASS':'FAIL',isolated)+'</div>';
    let gate=document.getElementById('v37-gate');
    if(!gate){gate=document.createElement('div');gate.id='v37-gate';gate.className='panel';document.querySelector('#test > .panel')?.appendChild(gate);}
    const linked=result.podComplete&&Boolean(result.invoiceNumber),complete=linked&&result.balanced&&isolated;
    if(gate)gate.innerHTML='<div class="section-head"><div><div class="eyebrow">V3.7 acceptance</div><h2>Financial Close Gate</h2></div>'+tag(complete?'FOUNDATION PASS':'IN PROGRESS',complete)+'</div><div class="metric-row"><span>POD, invoice, and Load ID linked</span>'+tag(linked?'PASS':'PENDING',linked)+'</div><div class="metric-row"><span>Double-entry journal balances</span>'+tag(result.balanced?'PASS':'FAIL',result.balanced)+'</div><div class="metric-row"><span>Second-load values cannot leak into this load</span>'+tag(isolated?'PASS':'FAIL',isolated)+'</div>';
  }
