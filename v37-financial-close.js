(() => {
  ...
  function reconcile(load){
    ...
    return {loadId:load?.id||'',podComplete:Boolean(load?.pod&&load?.deliveryProof?.signature),invoiceNumber:load?.invoice?.number||'',invoice,payments,receivable,expenses,profit:invoice-expenses,debits,credits,balanced:debits===credits,linesEqualInvoice,journal};
  }
