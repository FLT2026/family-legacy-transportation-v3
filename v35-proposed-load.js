...
(() => {
  ...
  function truckReady(x){return Boolean(x?.status==='active'&&validVin(x.vin)&&x.weightBasis==='scale-ticket'&&validScaleDate(x.verificationDate)&&Number(x.gvwr)>0&&Number(x.gcwr)>0&&Number(x.emptyWeight)>0&&Number(x.frontGawr)>0&&Number(x.rearGawr)>0&&Number(x.tireCapacity)>0&&Number(x.hitchCapacity)>0&&Number(x.emptyWeight)<Number(x.gvwr)&&Number(x.gvwr)<=Number(x.gcwr))}
