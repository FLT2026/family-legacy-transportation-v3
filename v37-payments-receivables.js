(() => {
  ...
  const closeLocked=load=>load?.financialClose?.status==='Closed';
  const closeReason='This load is financially closed. Use a protected correction workflow before changing payments.';
  ...
  function recordPayment(load,input,options={}){
    if(closeLocked(load))return {ok:false,reason:closeReason};
  ...
  function openPaymentForm(load){
    if(closeLocked(load)){if(typeof toast==='function')toast(closeReason);return}
  ...
  function render(){
    if(typeof current!=='function')return;const load=current(),locked=closeLocked(load),migrated=locked?false:normalizePayments(load);if(migrated&&typeof persist==='function')persist();const state=summary(load);
    ...
    const rows=state.records.length?state.records.map(record=>'<tr><td>'+esc(record.date)+'</td><td><strong>'+esc(record.id)+'</strong><br><span class="subtle">'+esc(record.method)+(record.reference?' · '+esc(record.reference):'')+'</span></td><td>'+esc(record.type)+(record.reversesPaymentId?'<br><span class="subtle">Reverses '+esc(record.reversesPaymentId)+'</span>':'')+'</td><td style="text-align:right"><strong>'+currency(record.amount)+'</strong></td><td style="text-align:right">'+(!locked&&record.type==='Payment'&&!state.records.some(item=>item.reversesPaymentId===record.id)?'<button class="btn danger" data-v37-reverse="'+esc(record.id)+'">Reverse</button>':'')+'</td></tr>').join(''):'<tr><td colspan="5" class="subtle">No payments recorded for this invoice.</td></tr>';
    panel.innerHTML='<div class="section-head"><div><div class="eyebrow">V3.7 payments & receivables</div><h2>'+esc(state.status.replaceAll(' ',' '))+'</h2><p class="subtle" style="margin-top:5px">Every payment and reversal remains allocated to '+esc(state.invoiceNumber||'the finalized invoice')+'.</p></div>'+(locked?'<span class="tag">CLOSED</span>':'<button class="btn primary" id="v37-record-payment">Record payment</button>')+'</div><div class="grid stats"><div class="stat"><span class="label">Invoice</span><div class="value">'+currency(state.invoice/100)+'</div></div><div class="stat"><span class="label">Net payments</span><div class="value">'+currency(state.payments/100)+'</div></div><div class="stat"><span class="label">Receivable</span><div class="value">'+currency(state.receivable/100)+'</div></div></div><table><thead><tr><th>Date</th><th>Payment ID</th><th>Type</th><th style="text-align:right">Amount</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>';
    ...
    const legacyButton=document.getElementById('record-payment');if(legacyButton){const replacement=legacyButton.cloneNode(true);replacement.textContent=locked?'Payments closed':'Record payment';replacement.disabled=locked;legacyButton.replaceWith(replacement);if(!locked)replacement.addEventListener('click',()=>openPaymentForm(load));}
