(() => {
  ...
  const hasDoc=(load,types)=>docs(load).some(doc=>doc?.status!=='Removed'&&types.includes(doc?.type)&&doc?.loadId===load?.id);
  function sourceRequirements(load){
    ...
    if(type==='Auto Auction'&&!hasDoc(load,['Auction Release / Gate Pass']))missing.push('Attach the auction release or gate-pass evidence for this Load ID.');
    if(['Load Board','Load App','Broker / Dispatcher'].includes(type)&&!hasDoc(load,['Rate Confirmation','Load Board / App Confirmation','Other Source Confirmation']))missing.push('Attach the rate/load confirmation for this Load ID.');
    if(['Direct Customer','Internal / Own Customer'].includes(type)&&!hasDoc(load,['Customer Order / Work Order','Other Source Confirmation']))missing.push('Attach or record the customer order/work authorization for this Load ID.');
    return missing;
  }
  function complianceRequirements(input={}){
    ...
    if(transportation.includes('Auto Transport')&&!hasDoc(load,['Vehicle Condition / Inspection','Pickup Photos']))missing.push('Auto Transport requires pickup condition evidence or pickup photos tied to this Load ID.');
    if(/Equipment|Machinery|Hotshot|Flatbed/i.test(transportation)&&!hasDoc(load,['Equipment / Securement Verification','Pickup Photos','Other Compliance Evidence']))missing.push('Equipment/flatbed transport requires securement or pickup-condition evidence tied to this Load ID.');
    if(load.requiresPermit===true&&!hasDoc(load,['Permit / Special Authorization']))missing.push('Required permit or special authorization is missing for this Load ID.');
  ...
  window.FLTDocumentCompliance={sourceTypes,transportationTypes,docTypes,sourceRequirements,complianceRequirements,evaluate,addDocument};
})();
