(() => {
  ...
  function resolve(input={}){
    ...
    return{
      ...
      customer:{name:first(load.customer,load.customerName),address:first(load.customerAddress,load.billToAddress),phone:first(load.customerPhone),email:first(load.customerEmail)},
      route:{pickup:first(load.pickup,load.pickupLocation),delivery:first(load.delivery,load.deliveryLocation),pickupDate:first(load.pickupDate,load.date),deliveryDate:first(load.deliveryDate)},
      ...
      operations:{pickupProof:load.pickupProof||null,deliveryProof:load.deliveryProof||null,actualTrip},
      finance:{revenue,totalExpenses,totalPayments,receivable:Math.max(0,revenue-totalPayments),profit:revenue-totalExpenses,invoice,expenses,payments}
    };
  }
