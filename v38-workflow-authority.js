(() => {
  ...
  function fleetReady(){
    ...
    const regularTrailer=(fleet.trailers||[]).find(item=>item.id===regular.trailerId&&item.status!=='cancelled');
    if(regularDriver&&regularTruck&&regularTrailer&&driverReady(regularDriver)&&truckReady(regularTruck)&&trailerReady(regularTrailer))return true;
