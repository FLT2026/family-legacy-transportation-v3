(() => {
  ...
  panel.innerHTML=`
    ...
    <form id="v36-actual-form" class="form-grid">
      ...
      <div class="field"><label for="v36-fuel-cost-input">Fuel receipt total</label><input id="v36-fuel-cost-input" name="fuelCost" type="number" min="0.01" step="0.01" inputmode="decimal" required></div>
      ...
      <label class="checkbox-field field full"><input name="noTollsIncurred" type="checkbox"> No tolls incurred for this trip</label>
      <div class="form-actions field full"><button class="btn" id="v36-use-estimate" type="button">Use Approved Estimate</button><button class="btn" id="v36-post-fuel" type="button" hidden>Post Fuel to Ledger</button><button class="btn primary">Save Actual Trip Snapshot</button></div>
    </form>
    ...
    <div class="panel" style="margin-top:14px"><div class="section-head"><div><h3>Estimate-to-actual reconciliation</h3><p class="subtle">Approved estimates stay unchanged; confirmed ledger costs become actuals.</p></div><span class="tag gray" id="v36-reconciliation-status">PENDING</span></div><div id="v36-reconciliation"></div></div>`;
  ...
  function openFuelExpense(record){
    if(!record||!window.FLTExpense?.open){toast('Save the actual trip before posting fuel.');return}
    window.FLTExpense.open({category:'Fuel',amount:Number(record.fuelCost).toFixed(2),date:current().deliveryDate||current().date||new Date().toISOString().slice(0,10)},{title:'Confirm fuel expense',record:{source:'actual-trip-fuel',linkedSnapshotId:record.snapshotId},success:'Fuel expense posted to the ledger.'});
  }
  function renderReconciliation(load,estimate){
    const costs=estimate.snapshot?.costs||{},rows=[
      ['Fuel',number(costs.fuel),actualFor(load,['Fuel'])],
      ['Driver pay',number(costs.driverPay),actualFor(load,['Driver pay'])],
      ['Tolls',number(costs.tolls),actualFor(load,['Tolls'])],
      ['Mileage and fixed reserves',number(costs.mileageCosts)!==null?Number(costs.mileageCosts)+Number(costs.insuranceAllocation||0):null,actualFor(load,['Maintenance','Tires','Insurance / Fixed'])],
  ...
  function render(){
    ...
    renderReconciliation(load,estimate);
  ...
  function renderGate(){
    const load=current(),record=latest(load),estimate=estimateFor(load),actualCost=ledgerTotal(load),checks={
      ...
      'Toll status and operating expenses recorded':(tollTotal(load)>0||Boolean(record?.noTollsIncurred))&&actualCost>0,
