(() => {
  ...
  function repairVisibleGate(){
    if(!document.getElementById('v36-gate')&&!document.getElementById('v37-gate'))return;
    const load=currentLoad();if(!load)return;
    migrateActualTripEvidence(load);repairV36Gate(load);repairV37Gate(load);
  ...
  // Do not observe the entire document or rerender Finance here. That caused visible scroll jitter.
